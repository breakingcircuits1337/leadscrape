    ...
    def run_training(self, backend_name='ibm_brisbane', num_iterations=10, population_size=8, shots=512, task_kwargs=None):
        ...
        with Session(service=service, backend=backend) as session:
            ...
            for iteration in range(num_iterations):
                ...
                print(f"Best: {best_fitness:.4f} | Avg: {np.mean(fitness_scores):.4f} | Mode: {adaptation_mode:11s} | σ: {self.mutation_strength:.4f} | λ: {self._compute_lyapunov_exponent():.4f} | Time: {iter_time:.2f}s")
